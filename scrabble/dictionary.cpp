...
bool Dictionary::is_word(const string& word) const {
    ...
    if(cur->is_final) {
        return true; //It is only a word if its is_final bool is set to true
    }
    return false;
}
...
shared_ptr<Dictionary::TrieNode> Dictionary::find_prefix(const string& prefix) const {
    ...
    for (char letter : prefix) {
        ...
        if(cur->nexts.find(letter) == cur->nexts.end()) {
            return nullptr; //We can return nullptr whenever we don't find a letter in the nexts map
        }
...
vector<char> Dictionary::next_letters(const std::string& prefix) const {
    ...
    for(auto const& pair : cur->nexts) {
        nexts.push_back(pair.first); //simply push each next char to a vector
    }
    return nexts;
}
